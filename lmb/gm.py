...
class GM():
    ...
    def __init__(self, params, x0=None, prior_mc=None):
        ...
        # define the gaussian mix comp
        if prior_mc is not None and prior_mc.dtype == self.dtype_mc:
            self.mc = prior_mc
        else: # 如果没有定义 mixture components, 默认一个高斯
    ...
    def predict(self):  # predict PDF below with x and P
        """
    ...
    def correct(self, z, z_id):
        ...
        if z is None or len(z) == 0:
            ...
            self.z_id = z_id
            for i, cmpnt in enumerate(self.mc): # each assignment has i mixture component and i pdfs
                ...
                # self.mc[i]['log_w'] 是个1D确定的数， 用multivariate_normal.logpdf 是[(2pi)°k*det(C)]°(-1/2)*(x-miu)C°（-1）(x-miu)， 相当于两个维度的data x被合成一个
                # x,P from predicted steps, now corrected and with each given x,P calculated multivariate PDF is log_w..

                mlpdf = multivariate_normal.logpdf(np.dot(self.H, cmpnt['x']), mean=z, cov=S)
                self.plot_xy.append([self.mc[i]['x'][0], self.mc[i]['x'][1]])
                self.mlpdf.append(mlpdf)
                self.mc[i]['log_w'] = cmpnt['log_w'] + mlpdf
    ...
    def overwrite_with_merged_pdf(self, pdfs, log_hyp_weights):
        ...
        mcs = [pdf.mc for pdf in pdfs] # e.g. 8 assignments, (8 hypothesis)
    ...
    def _merge_gaussians(self):
        ...
        into one mixture component by averaging their mean and covariance weighted
    ...
    def _prune_gaussians(self):
        ...
        # 存在问题， 如果prune_mask = [False, F, ..., F] 全部F， self.mc = [], log为0
        if True not in prune_mask:
            prune_mask[np.argmax(self.mc['log_w'])] = True
        ...
        # value error here: because log_w is empty
        #try:
         #   self.log_w_sum = logsumexp(self.mc['log_w'])
        ...
        '''if self.mc is None:
            ...
            a = np.exp(self.mc['log_w'])
            b = np.sum(a)
            self.log_w_sum = np.log(b)'''

        self.log_w_sum = logsumexp(self.mc['log_w'])
        self.mc['log_w'] -= self.log_w_sum


'''r = sum(assignment_weights)
# 0. Set existence probability to minus infinity to discard this target
# it is likely false target because hypothesis weights that associate with each measurement is too low
if r < 1e-9:
    self.log_r = -np.inf
    # self.pdf.mc = np.empty(0)
    return
'''
